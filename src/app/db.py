...
def add_restaurant(session: Session, name: str, macro_table_link: str, macro_table_format: str, notes: str, menu_link: str) -> Restaurant:
...
def list_foods_single_restaurant(session: Session, restaurant_id: int):
    results = session.exec(select(Food).where(Food.restaurant_id == restaurant_id)).all()
    return results


def get_update_create_restaurant(session: Session, name: str, macro_table_link: str, macro_table_format: str, notes: str, menu_link: str):
    query_result = session.exec(select(Restaurant).where(Restaurant.name == name)).first()
    if query_result:
        if (query_result.macro_table_link == macro_table_link and query_result.macro_table_format == macro_table_format and query_result.notes == notes and query_result.menu_link == menu_link):
            return query_result
        else:
...
def get_update_create_food(session: Session, food_name: str, size: str | None, restaurant_id: int, kcal: float, protein: float, fats: float, carbs: float):
    ...
    if query_result:
        ...
        return add_food(session, food_name, size, restaurant_id, kcal, protein, fats, carbs)
